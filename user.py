import requests,getpass,os,re,wmi,hashlib,pyperclip
...
class Upload(QtCore.QThread):
    ...
    def upload(self,AG):
        c=requests.post(f'{url}/sentScids',data=AG.encode("utf-8"),headers=header)
        msg=c.text
        c.close()
        return msg
    def run(self):
            ...
            with open(f'{username}\\PopstarDevs\\2Take1Menu\\cfg\\scid.cfg','a',encoding='utf-8') as f:
                for i in cloud_msg:
                    if i not in all_local_info:
                        ...
                        fl+=1
                    all+=1
            self._signal.emit(f'成功下载广告机{sc}个 本地重复广告机{fl-1}个 服务器云端广告机个数:{all-1}')
                        ...
                        if len(x[i])==3 and x[i][2]=='c':
                            if 9>=len(x[i][1])>=5:
                                if re.match('.*[A-Z].*',x[i][1]):
                                    er+=1
                                    all_info=all_info.replace(x[i][0]+':'+x[i][1]+':'+'c\n','')
                            else:
                                er+=1
                                all_info=all_info.replace(x[i][0]+':'+x[i][1]+':'+'c\n','')
...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "2Take1广告机云黑 V10"))
        self.label_2.setText(_translate("MainWindow", "每个人可以申请一个白名单，欢迎加入2T交流群872986398"))
    ...
    def changeTxt(self,msg):
        ...
        self.t3=None
        self.t4=None
        self.label.setText(QtCore.QCoreApplication.translate("MainWindow",msg))
...
ui.label_3.setText(QtCore.QCoreApplication.translate("MainWindow", "正在获取CPUid"))
for cpu in wmi.WMI().Win32_Processor():
    cpuid=cpu.ProcessorId.strip()
ui.label_3.setText(QtCore.QCoreApplication.translate("MainWindow", f"获取完成CPUid:{cpuid}"))
ui.label_3.setText(QtCore.QCoreApplication.translate("MainWindow", f"开始加密CPUid"))
cpuid=enj(cpuid)
ui.label_3.setText(QtCore.QCoreApplication.translate("MainWindow", f"加密完成CPUid:{cpuid}"))
ui.label_3.setText(QtCore.QCoreApplication.translate("MainWindow", f"开始获取主板id"))
for b in wmi.WMI().Win32_BaseBoard():
    boardid=b.SerialNumber
ui.label_3.setText(QtCore.QCoreApplication.translate("MainWindow", f"获取完成主板id:{boardid}"))
ui.label_3.setText(QtCore.QCoreApplication.translate("MainWindow", f"开始加密主板id"))
boardid=enj(boardid)
ui.label_3.setText(QtCore.QCoreApplication.translate("MainWindow", f"加密完成主板id:{boardid}"))
ui.label_3.setText(QtCore.QCoreApplication.translate("MainWindow", f"开始生成uuid"))
...
ui.label_3.setText(QtCore.QCoreApplication.translate("MainWindow", f"如有问题请带上你的UUID:{uuid}"))
